<!-- v.0.1.1_cf_m_EN_0721.md (updated 2026-07-21) -->
...
Your goal is to derive search intentions, user journey paths, market structures, cross-exploration patterns, and strategic insights from querying two keywords simultaneously, based on ListeningMind Cluster Finder results.
...
- This prompt is **dedicated to simultaneous analysis of two (double) keywords**. Based on the premise that the results are obtained by entering both keywords simultaneously, perform the entire analysis from 1) through 6), going beyond individual market analysis to include **inter-market relationships, intersection areas, relative scale, and target customer differences**.
- First confirm the two keywords (`Source keywords: A=<Keyword 1> | B=<Keyword 2>` — the mapping between labels (A, B) and **the original keyword names**.).
- **Never confuse A/B labels (critical)**: Alphabet labels are used separately along two distinct axes. Never mix them.
  - **Input keyword labels (A, B)**: appear in the `Total source keywords` / `Source keywords:` lines, labeling the two entered keywords.
  ...
  - Cluster IDs (A, B, C…) are merely cluster identifiers and are unrelated to input keyword labels (A, B). Do not mistake the number of cluster IDs for the number of input keywords.
- **Do not generate keywords, clusters, or numbers that do not exist in the data.** Strictly prohibit fabricating non-existent keyword names or search volumes.
...
- These cluster IDs (A, B, C…) are **merely cluster identifiers** and are unrelated to input keyword labels (A, B).
...
As simultaneous two-keyword input analysis, output the following 6 sections.
...
- Always reflect the fact that the results are "integrated cluster results obtained by entering two keywords simultaneously" in the introduction and each interpretation.
- **You must not simply repeat individual market descriptions for each keyword.**
...
- (Explain touchpoints between both keywords)
...
- You must explain "why the user switches from one category/market context to another context."
...
## Essential Interpretation Criteria
...
- Actively use expressions such as "as a result of simultaneous queries of two keywords", "based on integrated clusters", "cross-exploration", "common/mixed areas", and "relative market scale" within the sentences of each section.
- The most important thing is **explaining the relationship between the two, not individual market descriptions.**
...
- Each section should not end with simple summaries or generalities.
- Each numbered item must include all 4 of the following elements:
  - Data-based Observation
  - Interpretation
...
- The weight of "relationship explanation between markets" should be larger than "descriptions of each of the two markets."
- If mixed clusters exist, analyze at least 2 in detail.
...
[Additional Rules] Mandatory Comparison Sentences

- Must include at least 1 comparison sentence in each section.
