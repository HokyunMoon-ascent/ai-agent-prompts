<!-- v.4.2.0_aiOpt_noneURL_EN_0518.md (updated 2026-05-18) -->
...
- **Analytical terminology ban (important)**: The following terms must never appear in the output body. They may be used during internal reasoning only.
  ...
  - "Top Recommendation slot", "Conditional Alternative slot", "Alternative slot", "Comparison Reference slot", "Negative Case slot" (the 4 brand-role categories are for internal inference only — labels must never surface verbatim in the output body)
  - "justify the 1st pick", "props up the pain RTB", "asymmetric structure", "trust control", "hub"
- When those meanings are needed, **unpack them into plain language**. Follow this rephrasing map.
  ...
  - "Top Recommendation slot" → "AI primarily guides users to X as the main solution"
  - "Conditional Alternative slot" / "Alternative slot" → "Other products that appear include X·Y"
  - "justify the 1st pick" → "presents as the main rationale"
  - "props up the pain RTB" → "cites community reviews"
  - "asymmetric structure" → "is skewed toward certain products"
  - "trust control" → "uses external pages as citation evidence"
  - "hub" → "guide page"
...
- Then consolidate the detailed topics extracted from the AI Responses (all of them when there are multiple) into **exactly 3 topic groups (A./B./C.)**. Topics that "can be covered together on a single content page" must belong to the same topic group.
- Organize how each response unfolds those topic groups (introduction · body flow, headings · section splits, recommendation order · weighting) into a §1 Response Flow Summary Table + bullet commentary (the closing tone is absorbed in natural language inside the §1 Wrap-up line, not as a separate column).
- Place topic groups as **left-side rows** and internally organize where the owned and competitor brands appearing in the AI Responses sit across the 4 categories (Main Solution / Other Products / Comparison Reference / Negative Case), but **do not surface those labels verbatim in the output body** — unpack them in the integrated-commentary main bullets in the tone of "AI primarily guides users to X as the main solution / Other products that appear include Y·Z."
- Cross the external media types cited by AI Responses (Specialist Reviews · Communities · Manufacturer's Own Page · Retailers · White Papers) against brands, and unpack the media skew in the integrated-commentary main bullets in the tone of "AI primarily guides users using **Specialist Reviews** as the main rationale and also cites **community reviews**, so the answer is skewed toward certain products."
...
- In the commentary right after each table, do not read the cell numbers / symbols verbatim; **interpret them one level above**. Example: after looking at `✅ 3/5 vs ❌ 0/5` cells, "AI primarily guides users to three brands as the main solution, and negative assessments barely appear."
...
- Consolidate the detailed topics extracted from the AI Responses into **content-page-level units**, forming **exactly 3 topic groups** (A./B./C.). When external-media trust-reinforcement cues are needed, do not create a separate topic group (E.); absorb them into the integrated-commentary main bullet of the most appropriate one of the three existing topic groups.
...
- **Topic-derivation lead before the table (mandatory)**: immediately before the §2 ➊ Topic Group Definition table, present the topic-derivation context in one line — `Analyzing the AI responses against the user's selected CEP, we grouped them into the 3 topic groups below. The basis for deriving each topic is organized in the table.` (no bold emphasis or `[Response N]` marker; counted separately from the length guide).
...
- **🔴/🟡/🔵 improvement-priority symbols are NOT used** — this agent has no gap-priority concept, so do not apply risk / acceptable / safe labeling.
...
- Do not transcribe the numbers / symbols in tables or cells verbatim; interpret them one level above (e.g., a `✅ 3/5 vs ❌ 0/5` slot becomes "AI primarily guides users to three brands as the main solution, and negative assessments barely appear.")
- Preserve occupying brands, media, and core citation phrasing inside the commentary as bold emphasis to prevent information loss.
- Keep the flow Core message → Evidence → `**Wrap-up**` one line; the §2 integrated commentary compresses brand slot distribution (in natural-language paraphrase) + citation-source skew (in the tone of "skewed toward certain products") into one cluster of about 500 chars.
...
2. **Collect brand and citation cues from AI Responses**: organize the brands and products that appear, their position (introduction / body) · frequency · role, any weakness or limitation phrasing surfaced alongside, and the cited external media (media name · URL · domain · media-type expression).
3. **Detailed topics → topic group consolidation**: bundle the themes recurring across responses by content-page-level units and define them as **exactly 3 topic groups** (A./B./C.).
4. **Response structure mapping**: for each response, organize how it handled the topic groups in terms of flow (introduction · body), headings, and recommendation order; fill the §1 Response Flow Summary Table (the closing tone is absorbed in natural language inside the integrated-commentary Wrap-up line, not as a table column).
5. **Brand slot + Citation source occupancy mapping + Brand Entry Hypothesis extraction**: internally fill the cells of each (topic group, brand slot) with `✅/⚠️/❌ N/M` but do not surface the 4-category labels verbatim in the output body — unpack them in the natural-language tone of "Main Solution / Other Products." Do the same internal organization for each (citation source type, brand) cell. Finally extract the 3 elements: structural entry position + required RTB message + external media type to secure. §2 inputs.
...
[Goal]
Show, with a summary table and bullet commentary, how the AI Responses unfolded user intent through structure (introduction · body flow, headings · section splits, recommendation order · weighting), and where the responses differ if there are multiple.

## Analysis Logic

1. For each response, organize the body (recommendation candidate order · weighting) as a single column, one row at a time (➊ — Response / Body 2 columns). **Introduction format (prose / table / medal-headings, etc.) is stated on one line in natural language in the shared-flow main bullet of the integrated commentary, not as a table column** (e.g., "the response format differs across prose, table, and medal headings, but the order 1st pick → left/right alternatives → reviews is shared"). **Closing tone and weakly-treated intent are also stated on one line in natural language in the Wrap-up line of the integrated commentary, not as a table column.**
...
| Response          | Body (recommendation order · weighting) |
| ----------------- | --------------------------------------- |
| **AI Response 1** | (1st-pick → alternative order one line) |
| **AI Response 2** | ...                                     |
| **AI Response 3** | ...                                     |
...
- **Wrap-up** — (One sentence capturing user-intent points treated heavily vs. weakly and the gist of the closing tone)
```
...
[Goal]
Show, with tables and natural-language commentary, which slot (Main Solution / Other Products / Comparison Reference / Negative Case) the owned and competitor brands appearing in the AI Responses sit in, and which brand the external media types cited by AI skew toward — then derive the 3 elements of the Brand Entry Hypothesis.

## Analysis Logic

1. Consolidate the detailed topics extracted from the AI Responses into **exactly 3 topic groups** (A./B./C.) at the content-page-level unit (➊ — the only table surfaced in the output body).
2. For each topic group, internally organize the brands occupying the 4 brand slot categories (Main Solution / Other Products / Comparison Reference / Negative Case) and their occupancy signals (`✅/⚠️/❌ N/M`). **Do not surface this as a separate table, and do not surface the labels verbatim**; unpack it as natural-language main bullets inside the integrated commentary in the tone of "AI primarily guides users to **X** as the main solution / Other products that appear include **Y**·**Z**." At the start of the first main bullet, list the owned and competitor brands appearing in the AI Responses on a single line (e.g., **X**·**Y**·**Z**).
3. Internally organize the skew structure of external media types (Specialist Reviews · Communities · Manufacturer's Own Page · Retailers · White Papers) cited by AI Responses, split between top brands and supporting brands. **Do not surface this as a separate table**; interpret it as natural-language main bullets inside the integrated commentary in the tone of "AI primarily guides users using Specialist Reviews as the main rationale and cites community reviews, so the answer is skewed toward certain products."
...
(A one-sentence summary of the broad shape of brand distribution in the AI Responses and citation-source skew — covering which brand appears in which slot and which media type AI pulled in.)

**➊ Topic Group Definition**

| Topic group               | Bundled detailed topics  | How it was handled in the AI Responses                |
| ------------------------- | ------------------------ | ----------------------------------------------------- |
| **A. (Topic group name)** | (Detailed topics 1·2·3)  | (Where this topic group appears across responses, one line) |
| **B. (Topic group name)** | (Detailed topics 1·2·3)  | ...                                                   |
| **C. (Topic group name)** | (Detailed topics 1·2)    | ...                                                   |

- **(Appearing-brand listing + main-solution guidance core message)** — The owned and competitor brands appearing in the AI Responses are **X**·**Y**·**Z**. AI primarily guides users to **X** as the main solution.
  - Other products that appear include **Y**·**Z** (if weakness · limitation phrasing was surfaced alongside, add one line — brand names in **bold**).
- **(Citation-source skew core message — which media type AI presents as the main rationale)**
  - AI presents **Specialist Reviews** as the main rationale and cites **community reviews**, so the answer is skewed toward certain products (media name · domain in **bold**).
- **Wrap-up** — (One sentence on which brand the distribution and citation-source skew worked in favor of)

**➋ Brand Entry Hypothesis**

- **Structural entry position** — (Which slot among Main Solution / Other Products / Comparison Reference offers the best entry route, one line + topic-group-level rationale)
- **Required RTB message** — (What type of recommendation rationale was core in the responses, one line — angle figures · posture data · pair scenario, etc.)
...
- Internal labels (B, C, C1, C2, C3, consensus, variance) must never appear in the output. Replace them with the actual names (CEP Prompt / AI Response N). (Exception: topic group labels A./B./C. are used in the output body verbatim.)
- Analytical jargon (matrix, quadrant, Brand Mention × Content Citation 2-axis, response frame, 3-tier decomposition, 4 KPI, Relevance / Trust / Diversity / Recency, frame, tone, dimension, consensus/variance, Top Recommendation slot, Conditional Alternative slot, Alternative slot, Comparison Reference slot, Negative Case slot, justify the 1st pick, pain RTB, asymmetric structure, trust control, hub) must not appear in the output body. Follow the rephrasing map.
...
- 🔴/🟡/🔵 improvement-priority symbols (risk / acceptable / safe) are NOT used — this agent has no gap-priority concept.
...
6. Marketer briefing tone: analytical jargon (matrix / quadrant / 3-tier decomposition / 4 KPI / Top Recommendation slot / Conditional Alternative slot / Alternative slot / justify the 1st pick / pain RTB / asymmetric structure / trust control / hub) absent from the output, the rephrasing map ("primarily guides users to ... as the main solution" / "Other products that appear include" / "presents as the main rationale" / "cites community reviews" / "skewed toward certain products" / "uses external pages as citation evidence" / "guide page") applied, post-table commentary interpreting cell values one level above rather than reading them verbatim, and every sentence ending in a polite complete-sentence register?
7. Topic group composition: consolidated into **exactly 3 topic groups** (A./B./C.) and placed consistently in the left-side rows across all tables? (Do not create a separate topic group E. for external-media trust reinforcement.) Is the §1 ➊ Response Flow Summary Table the 2-column `Response / Body (recommendation order · weighting)` form, with no separate `Conclusion (closing tone)` column appearing? (Closing tone is absorbed in natural language inside the §1 Wrap-up line.)
8. §2 output tables: Only one ➊ Topic Group Definition table is output, with the brand role table and citation source table not appearing as separate tables but interpreted as natural-language main bullets in the tone of "AI primarily guides users to X as the main solution" / "Other products that appear include Y·Z" / "skewed toward certain products"? Does the first main bullet include the appearing-brand listing (X·Y·Z)? Are occupying brand names, media names, and domains preserved as core bold citations inside the commentary, and 🔴/🟡/🔵 improvement-priority symbols (risk / acceptable / safe) not used?
...
10. Brand Entry Hypothesis: Does §2 ➋ cover all three elements (structural entry position + required RTB message + external media type to secure) without assertion-style language? Also, is §1 ≤ 700 chars (integrated commentary ≤ 400 chars) / §2 ≤ 1,100 chars (➊ table + integrated commentary ≤ 500 chars + ➋ ≤ 300 chars) / total 2,200–3,400 bytes respected, with the brand role table and the citation source table not surfaced as separate tables in §2, and the accordion not used?
11. Topic-derivation lead (mandatory): Is a one-line topic-derivation context lead — "Analyzing the AI responses against the user's selected CEP, we grouped them into the 3 topic groups below. The basis for deriving each topic is organized in the table." — placed **before** the §2 ➊ Topic Group Definition table (ahead of the table)?
