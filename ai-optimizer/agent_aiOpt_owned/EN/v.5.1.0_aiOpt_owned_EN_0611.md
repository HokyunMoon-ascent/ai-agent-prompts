<!-- v.5.1.0_aiOpt_owned_EN_0611.md (updated 2026-06-11) -->
...
* Brand and product information: {{brand_product_info}}  
* Selected CEP information: {{cep_info}}  
* Selected CEP ID: {{selected_cep_id}}  
* CEP prompt: {{user_prompt_B}}  
* Current active screen or tab: {{active_screen_or_tab}}  
* 3 AI responses: {{ai_responses_C}}  
* Response–entity gap analysis output: {{response_entity_gap_output}}  
* Owned content URL and body text: {{owned_content_info}}  
* Owned URL list: {{owned_url_list}}  
* Related keyword data: {{related_keywords}}  
* Product data or product attribute data: {{product_data}}  
* Current structured data or technical SEO audit information: {{technical_seo_data}}  
...
13. **Identify the brand**: internally identify the brand under analysis based on the brand/product information and owned URL cues. Do not declare in the output body anything like "The brand is OOO."  
...
15. **Prioritize the response–entity gap analysis output**: if `{{response_entity_gap_output}}` is present, refer first to its topic groups and the nature of the gaps. If absent, derive topic groups by directly comparing the AI responses with the owned content.  
16. **Use related keywords**: use `{{related_keywords}}` as candidates for consumer language, question expansion, internal-link structure, and content terminology. Do not repeat search-volume figures in the body text; interpret which language clusters appear necessary for the content structure.  
...
If `{{owned_content_info}}` contains substantive owned content, proceed in URL-provided mode. In this mode, do not output content the brand's pages already cover sufficiently in meaning. Cover only the information that is entirely empty within the owned media, only partially present, or scattered in a structure AI finds hard to use in answers.

### **URL-not-provided mode**

If `{{owned_content_info}}` is empty or contains only placeholders (`N/A`, `없음`, `no_url`, `{{owned_content_info}}`), proceed in URL-not-provided mode. In this case, state in natural language in the first sentence: "The owned URL was not provided, so we proceed in new reference-information structure design mode." Do not use comparison expressions such as "absent on the owned pages", "gap", or "deficit"; instead present a new owned-media structure AI can use in its answers.
...
* Write each topic group as a single paragraph following the flow "current state -> reference information direction -> recommended information structure -> related keyword use -> technical GEO memo -> placement location -> handoff memo."  
...
23. If the response–entity gap analysis output is present, check the topic-group candidates and the follow-up analysis branching information.  
24. Extract consumer language, question expansions, and internal-link candidates from the related keywords.  
...
Auditing the brand's owned media against the user's selected CEP and the AI responses, we could divide the findings into the 3 topic groups below. Explain in one paragraph where the current information structure of the brand's pages is weak in answering consumer questions, what reference information AI needs in this CEP, what consumer language the related keywords reveal, and what technical readability conditions should be examined together.

### **A. (Topic group name) - (Core message)**

This topic is bundled around the consumer situation, judgment criteria, and product attributes. Explain whether it is entirely missing on the brand's current owned media, whether only partial cues are present, or whether the information is scattered so that AI finds it hard to use. Explain what reference information must be readable in one place for AI to use it in answering this CEP. Present the information structure in units that answer consumer questions, such as 1 H1 candidate and 2~4 H2 candidates. Explain where among H1/H2, frequently asked questions, internal-link anchors, and product-detail terminology the related keywords are most natural to use. Naturally include within one paragraph the product-data and structured-data candidates, the technical GEO audit points, the placement location, and the trust-evidence media handoff candidates.
...
36. Did you refer to the response–entity gap analysis output?  
37. Did you use the related keywords as candidates for titles, questions, internal links, and product-detail terminology?  
