<!-- v.2.1.1_phase5_핵심 구매 요인 도출_KR_0825.md
-->
...
- 그다음 그 일을 기준으로 제품 속성·제약 조건을 **최소 1개~최대 3개** 뽑습니다(반드시 1개 이상 도출). 각 KBF는 완전한 문장이 아니라 간결한 명사구입니다.
...
5. 각 CEP 상황마다 **최소 1개~최대 3개의 KBF**를 도출하세요.

- **주의: KBF가 0개인 상황은 허용되지 않습니다.** 카드의 근거(source_section_ids)를 깊이 분석하여 반드시 1개 이상의 유효한 KBF를 도출하되, 2~3개를 채우기 위해 근거 없는 속성을 억지로 추가하지는 마세요.
...
[
  ...
  },
  {
    ...
    "kbfs": ["배터리 지속 시간"],
    "kbf_evidence": [
      { "section_id": 2, "bullet_index": 0, "quote": "충전 없이 며칠은 썼으면 좋겠어요" }
    ]
  }
]
```
...
- `kbf_evidence`는 `kbfs`와 **길이와 순서가 같은** 배열입니다. (KBF 배열은 비어있을 수 없습니다.)
