<!-- v.2.2.0_phase5_핵심 구매 요인 도출_KR_0825.md
     v.2.1.0(0825) → v.2.2.0(0825)
     - KBF 정의를 "제품 속성이나 제약 조건" → "선택 기준"으로 확장. 비교 가능성 게이트는 그대로 유지.
     - 속성 축에 사후·공급 / 데이터·연결 / 위생·관리 3개 추가. 닫힌 목록 성격은 유지.
     - rtb 제약을 "신뢰 근거 전체 배제" → "문장 그대로 옮기기 금지"로 축소.
     배경: 제품 스펙 축 밖의 장면(사회적 체면·사후 서비스·프라이버시)에서 kbfs가 []로 비던 문제.
-->
...
3. KBF output 형식
   - 비교가 가능한 **선택 기준**으로 치환. 제품 자체의 속성일 수도 있고, 그 제품을 사서 쓰는 조건일 수도 있습니다.
   - 상황 서술을 그대로 옮기지 않는다.
   - 제품명이 사용되지 않는다.
...
- 정성 속성도 근거에 없으면 출력하지 않습니다.
  ...
  - 보관·처리
  - 구성·번들
  - 사후·공급 (보증 범위, 수거·수리 리드타임, 소모품 공급 구조)
  - 데이터·연결 (처리 위치, 계정 연동 범위, 오프라인 동작 가능 여부)
  - 위생·관리 (세척 구조, 건조 방식, 냄새 관리 구조)

- **이 목록은 닫혀 있습니다.** 여기 없는 축으로 칸을 채우지 마세요.
...
6. 제약 전달: 다음은 KBF로 사용되지 못합니다.
   - 카드의 `rtb`에 있는 문장을 그대로 옮긴 것 — 신뢰 근거를 KBF로 되풀이하지 마세요. 다만 `rtb`가 언급한 대상이라도 비교 가능한 기준으로 치환되면 KBF입니다.
