<!-- v.2.0.1_aiOpt_gap_EN_0506.md (updated 2026-06-01) -->
...
> 2. **CEP Prompt** is a single user question prompt. Use the Category Entry Point (CEP), Key Buying Factor (KBF), and Reason To Believe (RTB) cues embedded in the query as the baseline of the analysis.
...
- First extract the user intent (CEP / KBF / RTB) from the CEP Prompt and fix it as the analysis baseline.
...
- Base the judgment on the Owned Page's current center of gravity, page character, and alignment with the CEP Prompt.
...
2. **Extract semantic areas from AI Responses (per response)**: collect all entities (brands, products, attributes, figures, scenarios) and topics (review criteria, alternatives, decision guides) the response covers in order to satisfy the CEP Prompt's intent.
...
[Goal]
Compare at a glance the user intent of the CEP Prompt, the semantic areas the AI Responses commonly cover, and the Owned Page's center of gravity, and summarize the big picture of the gap in a single line.
...
**➊ Summary of CEP Prompt user intent (CEP)**
...
- Never expose internal labels such as A, B, C, C1, C2, C3, consensus, variance in the output. Replace them all with the actual names (Owned Page / CEP Prompt / AI Response N / Consensus area / Variance area).
