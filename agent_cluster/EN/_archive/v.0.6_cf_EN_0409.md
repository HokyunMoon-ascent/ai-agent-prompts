...
- First, **determine whether the number of input keywords is 1 or 2**.
- **When 1 keyword is input**, perform analysis according to the existing single-keyword analysis framework (Sections 1, 2, 3, and 4).
- **When 2 keywords are input**, based on the premise that the results are obtained by entering both keywords simultaneously, analyze and interpret beyond individual market analysis to include **relationships between markets, intersection areas, relative scales, and target customer differences** (Sections 1 through 6).
- All numerical values must use the actual values from the provided data (csv). Rounding, estimation, or arbitrary corrections are prohibited.
