<!-- v.0.7.1_cf_EN_0602.md (updated 2026-06-02) -->
...
- Before starting the analysis, first fix the **single seed keyword**. The seed keyword is the subject of the user's question; in the data, the `src` column is a single value, and the **hub (`h=TRUE`) keyword of cluster A (mapping 0)** corresponds to it. (e.g., :k[vertical mouse])
- All `src` values being identical means the input is **a single keyword (one)**. Even if brand/generic keywords with larger search volume exist in the data (e.g., :k[Logitech]), **do not replace the seed.** The seed is determined by the question subject / cluster A, not by the maximum search volume.
- The analysis scope is limited to **the same product as the seed itself** (its variants, brands/models, lineup, recommendations, reviews, price, purchase, usage).
- **Keywords/clusters of a different product category are completely excluded from analysis and output.** Criterion: if a cluster's core **product noun** differs from the seed, exclude it (but include the seed product's brand/model/lineup names even if the product noun is omitted). e.g., if the seed is :k[vertical mouse], then keyboards, mousepads, wrist rests, grips, etc. are separate product categories → do not include them in main categories, search-volume sums, flows, or insights. **Even if they exist in the data**, exclude them if they are not the seed product itself.
- Do not name or present any main category or cluster as an **independent market equal to the seed** (like "the ○○ market") or as a **second keyword**. Describe every main category as an "intention/demand axis within the seed keyword's market."
...
- In the first summary sentence, **explicitly name the seed keyword** and declare that "this analysis is the internal structure of the :k[seed keyword] single market."
...
(Total summary sentence here — the first sentence must state it is the :k[seed keyword] single market)
...
0. **First exclude clusters of a different product category from the seed product**, then proceed with the steps below.
...
3. Use only From→To paths that are **movements between clusters within the seed product**; exclude paths that pass through nodes of other product categories.
...
- Always analyze only **the single seed keyword's market (one)**. Do not output **different product categories** such as keyboards, mousepads, or wrist rests.
