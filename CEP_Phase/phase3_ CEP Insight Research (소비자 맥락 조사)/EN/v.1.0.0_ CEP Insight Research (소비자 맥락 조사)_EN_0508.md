...
## Phase 3 — CEP Insight Research (Consumer Context Research)

### Purpose

Research the **real-life contexts, situations, and triggers** that cause consumers to think of a specific product.  
The focus is on discovering CEPs (Category Entry Points), leveraging online communities/reviews/social media.

### Input Variables

| Variable                 | Description                                        | Example                |
| ------------------------ | ------------------------------------------------- | ---------------------- |
| `{{product_name}}`       | Product name or brand name                         | `갤럭시 S25 Ultra`     |
| `{{region}}`             | Target market                                      | `South Korea`          |
| `{{response_language}}`  | Response language                                  | `Korean`               |
| `{{research_date}}`      | Research reference date                            | `2026.02.27`           |
| `{{category}}`           | [Optional] Product category (Product Anchors result) | `스마트폰, 플래그십폰` |
| `{{community_examples}}` | Country-specific community examples                | (see below)            |

#### Country-Specific Community Examples (`{{community_examples}}`)
...
Markdown document (H1 title + up to 10 H2 sections)  
Each section consists of a number + insight sentence. 3 paragraphs per section.
...
| text.verbosity    | 'low'                                                                                           |
| reasoning         | if enableReasoningSummary is true, { effort: 'low', summary: 'concise' }, otherwise { effort: 'low' } |
