<!-- v.3.1.2_aiOpt_noneUrl_integrate_EN_0721.md (updated 2026-07-21) -->
...
At the start of the analysis, explain qualitatively to what degree the company's own brand appears within the AI responses. Do not write specific figures such as mention counts or the number of responses in which it appeared in the output; express the presence, role, and weight (e.g., appears in only some responses, appears as a peripheral alternative). **Use the table figures only as internal judgment, and do not recount them from the raw AI responses.**
...
11. Use the `self_mention` / `mention_comparison` figures only as internal judgment for the weight of the company's own brand mentions, and describe it in the output by presence and role rather than specific figures. Interpret the context of the mention (whether it is the center of the recommended candidate group or a peripheral alternative) from the raw responses.
...
(Describe qualitatively whether the company's own brand appears in the AI responses and in what role (representative candidate, conditional candidate, peripheral alternative, simple mention) it appears. Do not write specific figures such as mention counts, the number of responses in which it appeared, or per-response mention counts; express only the presence, role, and weight. Because this is the No-URL version, do not judge whether the company's own domain/URL was cited, and explain the composition of source types (official information, distribution, news, reviews, community, experts, etc.) qualitatively. Use the table figures only as internal judgment.)
...
- Do not write specific figures (mention counts, the number of responses in which something appeared, the number of responses in which something was cited, etc.) in the output. Describe whether and in what role the company's own brand appears qualitatively. Use the table figures only as internal judgment, do not expose them as numbers in the output, and do not recount them from the raw AI responses.
