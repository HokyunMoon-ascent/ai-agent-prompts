...
CEP마다 KBF·카테고리를 넣어 질문 1개 생성 (AI 검색용 자연어)

> 할루시네이션 억제 개정(0714): 나노인텐트가 Phase 4 출력에서 제거됨에 따라 `nanoIntents` 입력과 프롬프트 내 Nano Intent 참조를 제거했습니다.
> ⚠ 백엔드 정합 필요: `buildCepUserPromptsPrompt(...)`의 nanoIntents 인자·`${nanoIntentsText}` 조립 제거.
...
```
아래의 CEP(Category Entry Point) 상황, KBF(Key Buying Factor)를 사용해 실제 사용자가 AI 챗봇(ChatGPT, Gemini, Perplexity 등)에게 물어볼 법한 자연스러운 질문을 생성하세요.
...
질문은 완전한 이야기를 담아야 합니다: 사용자의 상황(주어진 CEP), 그리고 필요한 제품 특징(모든 KBF를 자연스럽게 엮은 것).
