...
The goal of owned media is not to say "our product is good" more loudly. The core is to make **why the brand is the answer in this CEP** readable to AI. Therefore, do not merely list product names, ingredients, prices, volumes, and usage instructions; "who is it suitable for", "what situational inconvenience does it solve", "how does it differ from similar alternatives", "what are the selection criteria", and "what official evidence can AI cite" must be connected within a single structure. Also, the same selection criteria and core evidence should not sit in only one product detail page; repeat them consistently across multiple owned channels such as frequently asked questions, guides, and official-store product information, so that AI confirms the same signal again and again.
...
- Write the detail of each topic group (inside the accordion) as a single paragraph following the flow "current state -> reference information direction -> recommended information structure -> consumer language use -> technical GEO memo -> placement location -> handoff memo."
...
(Summary 1~200 chars: in the first sentence, state as a conclusion the direction of reference information the brand must establish for this topic, then briefly add where the current structure is weak. Write outside the accordion, conclusion-first.)
...
(Summary 1~200 chars: conclusion-first. Outside the accordion.)
...
(Summary 1~200 chars: in the first sentence, state as a conclusion the direction of reference information the new owned media must establish for this topic. Write outside the accordion, conclusion-first.)
...
- Do not use tables.
- Compose each topic group as a summary (outside the accordion, 1~200 chars) + one accordion. There is exactly one accordion per topic, three per section. Do not write detail outside the accordion.
- Write each topic group as an H3 title, a one-line summary, and a single detail paragraph inside one accordion.
...
- Keep the total of the accordion-internal detail within 2,000~3,000 characters, and write each topic summary separately at 1~200 characters per topic.
...
48. Are there no tables or source markers, and did each topic keep the summary (outside the accordion) + one accordion (three per section) structure?
