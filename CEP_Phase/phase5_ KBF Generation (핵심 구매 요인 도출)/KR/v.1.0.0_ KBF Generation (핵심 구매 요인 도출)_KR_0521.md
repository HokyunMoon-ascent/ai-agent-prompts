...
> 할루시네이션 억제 개정(0714): 나노인텐트가 Phase 4 출력에서 제거됨에 따라, 본 프롬프트의 입력·정의에서 나노인텐트 참조를 제거하고 KBF를 CEP 상황 기반으로 재정의했습니다.
> ⚠ 백엔드 정합 필요: cep_situations 조립 시 nanoIntents 주입 제거.
...
````
<!-- v.1.0.0_ KBF Generation (핵심 구매 요인 도출)_KR_0521.md -->
# Role
...
**규칙:** 각 KBF는 **구체적인 제품 속성**이어야 하며 — 소비자 니즈를 바꿔 말한 것도, **지어낸 수치나 범위**도 아니어야 합니다. 특정 숫자는 Basic Product Research에 명시적으로 존재하는 경우에만 인용하십시오.
````
