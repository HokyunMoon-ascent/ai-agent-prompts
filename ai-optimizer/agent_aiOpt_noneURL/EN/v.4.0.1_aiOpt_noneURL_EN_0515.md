<!-- v.4.0.1_aiOpt_noneURL_EN_0515.md (updated 2026-05-15) -->
...
- Section-level length guide: Response Structure Analysis ≤ 700 chars (large-paragraph integrated commentary ≤ 400 chars) · Brand Mention Context & Citation Source Analysis ≤ 1,100 chars (integrated commentary right after ➊·➋·➌ tables ≤ 400 chars + ➍ Brand Entry Hypothesis ≤ 300 chars).
- Total output length guide: average 2,600–4,000 bytes, hard cap 5,500 bytes. Never pull in external facts / conjecture just to fill length (Source Lock takes priority).
...
### 10. Large-Paragraph Integrated Commentary Principle

- Do not place commentary between sub-tables within a large paragraph. **Output all tables consecutively, then place exactly one integrated commentary of about 400 characters at the end of the large paragraph** (§2 keeps the structure of integrated commentary 400 chars right after the ➊·➋·➌ tables + ➍ Brand Entry Hypothesis 300 chars).
- Main bullets carry 2–3 **core messages in bold**; sub-bullets carry evidence and detail in a hierarchy.
- Do not read the numbers / symbols shown in the table verbatim — interpret them one level above.
- Keep the flow Core message → Evidence → `**Wrap-up**` one line, and compress topic-group-level distribution and citation-source asymmetry into one integrated commentary.
...
5. Output the ➊·➋·➌ tables consecutively, then place exactly one integrated commentary (about 400 characters) right after ➌, followed by ➍ Brand Entry Hypothesis (about 300 characters) as a separate block. Do not place mid-commentary between sub-tables.

## Common Instructions

- Total within 1,100 characters. Integrated commentary right after ➊·➋·➌ tables ≤ 400 characters (no mid-commentary); ➍ Brand Entry Hypothesis ≤ 300 characters.
...
- **(Brand role slot core message — which brand is the 1st-pick standard, and how the Conditional · Comparison slots are scattered)**
  - (Evidence: in which response it rose to 1st pick with what RTB, and weakness · limitation phrasing surfaced alongside — one line)
- **(Citation-source asymmetry core message — which media type held up the 1st-pick slot)**
  - (Among Specialist Reviews · Community Reviews · Quantitative Data, the decisive media type — one line)
...
- Total body length follows the guide: average 2,600–4,000 bytes, hard cap 5,500 bytes.
...
10. Brand Entry Hypothesis: Does §2 ➍ cover all three elements (structural entry position + required RTB message + external media type to secure) without assertion-style language? Also, is §1 ≤ 700 chars (integrated commentary ≤ 400 chars) / §2 ≤ 1,100 chars (integrated commentary ≤ 400 chars + ➍ ≤ 300 chars) / total 2,600–4,000 bytes respected, no mid-commentary between sub-tables, and is the accordion not used?
