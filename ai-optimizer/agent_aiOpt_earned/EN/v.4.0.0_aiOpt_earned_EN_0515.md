<!-- v.4.0.0_aiOpt_earned_EN_0515.md (updated 2026-06-01) -->
...
2. **Topic Grouping**: Bundle the topics covered across the AI Responses into **4 to 5 groups in brand-content-production action units**, and extract a representative keyword for each group.
...
- **Left rows** = 4 to 5 topic groups (in brand-content-production action units). Append a representative keyword next to each topic in **bold**.
...
- After the table, place **bullet commentary of around 600 characters** (main bullets = key messages in bold, sub-bullets = evidence / details).

### Output 2: Topic-Level Action Matrix

- **Left rows** = topic groups (identical to Output 1).
- **Columns** = `Priority` / `Priority Channel` / `Trigger Content` / `Expressions AI Will Cite`, 4 columns (5 columns including the left rows).
- **Priority**: one of 🔴 Critical · 🟡 Opportunity · 🔵 Reinforcement.
- **Priority Channel**: 1 to 2 from the 5-channel classification (Media PR · Community · Reviewer · Expert Citation · Commerce).
...
- **Expressions AI Will Cite**: which attributes / numbers / experiential expressions AI will cite when it recommends the brand as a result (in **bold emphasis** form).
- After the table, place **bullet commentary of around 600 characters**.
...
- **Output 1 adaptation**: Replace the base columns with `Current Response Citation Distribution` / `Competitively Advantaged Brand`, 2 columns. The `Current Response Citation Distribution` cell uses ✅/⚠️/❌ + `N/M` + the cited channel / medium in **bold**; the `Competitively Advantaged Brand` cell lists 1 to 2 competitor brand names that the responses cite most frequently for that topic. The left rows remain identical: 4 to 5 topic groups.
- **Output 2 adaptation**: Keep the column and commentary structure unchanged. The `Expressions AI Will Cite` column must be derived only from expressions that actually exist in the AI Response.
...
- The left rows of every matrix must be 4 to 5 topic groups, bundled in brand-content-production action units. Do not expose the 5-channel classification as rows.
...
- Every action must also present "which expressions / numbers / testimonials AI will cite when it recommends the brand as a result". The `Expressions AI Will Cite` column in Output 2 must not be left blank.
...
| **(Topic Group 4)** **representative keyword** | ⚠️ N/M<br>**Medium**         | ⚠️ N/M<br>**Medium**                  |
...
- **(Topic Group 2 Key Message)**
  - (Evidence 1 · 2)
- **(Topic Groups 3 · 4 Key Message)**
  - (Evidence)
- **Overall** — (1-line diagnosis of the citation power landscape)

## Topic-Level Action

| Topic Group                            | Priority  | Priority Channel         | Trigger Content                                            | Expressions AI Will Cite       |
| -------------------------------------- | :-------: | ------------------------ | ---------------------------------------------------------- | ------------------------------ |
| **(Topic Group 1)** **representative keyword** | 🔴 Critical | Reviewer / Media PR    | (which medium · influencer · expert to trigger in which format) | **attribute · number · experiential expression** |
| **(Topic Group 2)** **representative keyword** | 🔴 Critical | Community / Reviewer   | (Trigger Content, 1 to 2 sentences)                            | **attribute · context-bearing expression**       |
| **(Topic Group 3)** **representative keyword** | 🟡 Opportunity | Expert Citation    | (Trigger Content, 1 to 2 sentences)                            | **quantitative expression**                      |
| **(Topic Group 4)** **representative keyword** | 🔵 Reinforcement | Commerce         | (Trigger Content, 1 to 2 sentences)                            | **experiential testimonial expression**          |

> 🔴 Critical (start immediately) · 🟡 Opportunity (2nd–3rd priority) · 🔵 Reinforcement (balanced citation recovery)

- **(Topic Group 1 Action Key Message — trigger-signal unit)**
  - (Evidence 1: how AI citation probability shifts when triggered to which medium in which format)
  - (Evidence 2: channel fit of the Expressions AI Will Cite)
- **(Topic Group 2 Action Key Message)**
  - (Evidence 1 · 2)
- **(Topic Groups 3 · 4 Action Key Message — interpretation of opportunity / reinforcement areas)**
- **Overall** — (1-line diagnosis of the priority trigger-signal axis)
```
...
2. **Number of topic groups**: Are the left-row topic groups within the 4 to 5 range?
3. **Column count cap**: Is the Output 1 matrix within 5 columns (topic group + up to 3), and the Output 2 matrix within 5 columns (topic group + 4)?
4. **Cell notation convention**: Do Output 1 cells follow the format ✅/⚠️/❌ + `N/M` + cited channel / medium in **bold**?
...
6. **5-channel classification usage**: Is 1 to 2 of the 5-channel classification (Media PR · Community · Reviewer · Expert Citation · Commerce) explicitly stated in the channel notation inside Output 1 cells or in the `Priority Channel` column of Output 2?
7. **'Expressions AI Will Cite' filled**: Is the `Expressions AI Will Cite` column in Output 2 non-empty for every topic group and notated in **bold emphasis** form?
8. **Field-specific analytical terms not exposed**: Do expressions such as "RTB", "directional alignment", "AI visibility inhibitor hypothesis", "consensus", "variance", "camp", "common area", "difference area" not appear in the body at all?
...
11. **Commentary length**: Does the bullet commentary immediately after each matrix conclude within around 600 characters (±150)?
