<!-- v.0.6.4_cf_EN_0601.md (updated 2026-06-01) -->
...
- **Input keyword count determination (top-priority rule)**: The number of input keywords is determined solely by the `Total source keywords` value in `### Input Information` above.
  - `Total source keywords: 1` → single keyword (4 sections). `Total source keywords: 2` → double keywords (6 sections).
  - This value must match the number of labels (A, B…) actually listed in the `Source keywords: A=… | B=…` line. If the two ever disagree, prioritize the `Total source keywords` value.
- **Never confuse A/B labels (critical)**: Alphabet labels are used separately along two distinct axes. Never mix them.
  - **Input keyword labels (A, B)**: appear only in the `Total source keywords` / `Source keywords:` lines, labeling the entered keywords (1–2 of them).
  - **Cluster IDs (A, B, C…)**: come from the `c` column (0→A, 1→B, 2→C…) and are cluster identifiers; the data may contain many clusters A·B·C….
  - **The fact that cluster B (or C, K, L, Q, etc.) exists in the data does NOT mean a second input keyword (Keyword B) exists.** A second input keyword exists only when `Total source keywords` is 2 AND `Source keywords` explicitly contains `B=…`.
- **When single keyword (`Total source keywords: 1`)**: Perform only Sections 1), 2), 3), 4) under the single-keyword framework. **Never output** Sections 5), 6) or any double-only phrasing such as "Keyword B", "simultaneous query of two keywords", "intersection area", or "integrated market".
- **When double keywords (`Total source keywords: 2`)**: First confirm the two keywords (`Source keywords: A=<Keyword 1> | B=<Keyword 2>` — the mapping between labels (A, B) and **the original keyword names**.). Based on the premise that the results are obtained by entering both keywords simultaneously, perform the entire analysis from 1) through 6), going beyond individual market analysis to include **inter-market relationships, intersection areas, relative scale, and target customer differences**.
- **Do not generate keywords, clusters, or numbers that do not exist in the data.** In particular, strictly prohibit fabricating a second input keyword market (a non-existent keyword name / search volume) in single-keyword analysis.
...
- These cluster IDs (A, B, C…) are **merely cluster identifiers** and are unrelated to input keyword labels (A, B). Do not mistake the number of cluster IDs for the number of input keywords.
...
> **[Mandatory self-check before output — keyword count gate]**
> 1. Read the `Total source keywords` value in `### Input Information`.
> 2. Count the number of labels (A, B…) in the `Source keywords` line.
> 3. Confirm the keyword count N indicated by the two signals (if they disagree, prioritize the `Total source keywords` value).
> 4. If N=1, output exactly 4 sections (1–4); if N=2, output exactly 6 sections (1–6).
> 5. If a single-keyword case starts generating Sections 5)·6) or a "Keyword B", that output is an error and must be removed immediately.
> 6. **Cluster IDs (the `c` column) must NEVER be used to determine the keyword count.**
...
**➊ Search Purpose Title 1 (Total Search Volume: NN / Number of Clusters: NN / Nature: single = internal market nature / double = Keyword A centered · Keyword B centered · Mixed)**
...
> **Output this section only for double keywords (`Total source keywords: 2`). Never write it for single-keyword input.**
...
- If `Total source keywords` is 1, output exactly 4 sections and do NOT output Keyword B, Intersection Area (5), or Individual Market Analysis (6).
- If `Total source keywords` is 2, output exactly all 6 sections.
