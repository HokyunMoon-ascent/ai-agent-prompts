<!-- v.4.0.1_aiOpt_gap_EN_0515.md (updated 2026-06-01) -->
...
- Section-level length guide: Brand Exposure Analysis ≤ 700 chars (consolidated commentary at the end of the major section ≤ 400 chars) · Semantic Gap Mapping ≤ 1,000 chars (consolidated commentary right after the ➊ table ≤ 400 chars + ➋ gap detail ≤ 500 chars).
- Total output length guide: average 2,800–4,200 bytes, hard cap 6,000 bytes. Never pull in external facts / conjecture just to fill length (Source Lock takes priority).
...
### 9. Consolidated Major-Section Commentary Principle

- Do not place commentary between sub-tables inside a major section. **Render all tables consecutively and place a single consolidated commentary of about 400 characters at the end of the major section.**
...
- Keep the flow Core message → Evidence → `**Wrap-up**` one-liner, and have the consolidated commentary compress topic-group camp gaps, citation-source asymmetry, and non-exposure hypotheses into a single bundle.
...
5. Cover the two or more deficiency signals explaining the Owned Brand's non-exposure / weak exposure briefly in the final main bullet of the consolidated commentary (no separate ➍ block).
6. Render the ➊·➋·➌ tables consecutively, then place a single consolidated commentary of about 400 characters at the end of the major section (no intermediate commentary).

## Common Instructions

- Total within 700 characters; consolidated major-section commentary about 400 characters (no intermediate commentary between sub-tables).
...
- **(Camp-gap core message per group — call out one or two decisive occupancy voids)**
...
- **(Citation-source asymmetry core message — where the trust-cue control resides)**
  - (One-line cue showing the Owned response leaning on external outlets)
- **Deficiency signals** — **Signal 1** (e.g. absence of quantitative comparison figures) · **Signal 2** (e.g. absence of review citations) in one plain-language line
...
10. Length guide: Brand Exposure Analysis ≤ 700 chars (consolidated major-section commentary ≤ 400 chars) / Semantic Gap Mapping ≤ 1,000 chars (consolidated commentary right after the ➊ table ≤ 400 chars + ➋ gap detail ≤ 500 chars) respected, and no intermediate commentary inserted between sub-tables?
